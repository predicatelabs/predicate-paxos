.PHONY: install tests clean format deploy-hook deploy-tokens-and-pool deploy-contracts
...
install:
	git submodule update --init --recursive && git config --local core.hooksPath .githooks/
...
deploy-tokens-and-pool:
	export NETWORK=LOCAL && \
	export HOOK_ADDRESS=0x3092Bf8B26C4CcB8d620B089a6a5e23D42d34080 && \
...
deploy-contracts: deploy-pool-manager deploy-hook deploy-tokens-and-pool
