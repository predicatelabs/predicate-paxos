...
contract DeployPredicateHook is Script {
    ...
    function run() public {
        ...
        bytes memory constructorArgs = abi.encode(config.poolManager, config.serviceManager, config.policyId);
        ...
        PredicateHook predicateHook =
            new PredicateHook{salt: salt}(config.poolManager, config.serviceManager, config.policyId);
