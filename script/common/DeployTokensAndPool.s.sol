...
contract DeployTokensAndPool is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        console.log("Deployed Swap Router: %s", address(swapRouter));
        vm.stopBroadcast();

        // test the lifecycle (create pool, add liquidity, swap)
        vm.startBroadcast();
        testLifecycle(manager, hookAddress, posm, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
    ...
    ) internal returns (PoolModifyLiquidityTest lpRouter, PoolSwapTest swapRouter) {
