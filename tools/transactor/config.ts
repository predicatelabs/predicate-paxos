...
export function validateConfig(cfg: Config): void {
  ...
  if (!cfg.currency0Address || !utils.isAddress(cfg.currency0Address)) {
    throw new Error("currency0 address is required and must be a valid Ethereum address");
  }
  if (!cfg.currency1Address || !utils.isAddress(cfg.currency1Address)) {
    throw new Error("currency1 address is required and must be a valid Ethereum address");
  }
  if (!cfg.routerAddress || !utils.isAddress(cfg.routerAddress)) {
    throw new Error("swap router address is required and must be a valid Ethereum address");
  }
  if (!cfg.hookAddress || !utils.isAddress(cfg.hookAddress)) {
    throw new Error("hook address is required and must be a valid Ethereum address");
  }
}
