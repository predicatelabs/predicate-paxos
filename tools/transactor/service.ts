...
import { STMRequest, STMResponse, PredicateMessage, PoolKey, SwapParams } from "./types";
...
export class TransactorService {
  ...
  constructor(private config: Config) {
    ...
    this.swapRouter = new ethers.Contract(this.routerAddress, swapRouterAbi, this.wallet);
  ...
  async start() {
    console.log(`Running transactor service in ${this.environment} environment`);
  ...
  async getHookData(params: SwapParams): Promise<string> {
    ...
    const dataBeforeSwap = this.encodeBeforeSwap(this.wallet.address, this.poolKey, params);
    ...
    const hookDataEncoded = this.encodeHookData(pm, this.wallet.address, BigNumber.from("0"));
  ...
  encodeBeforeSwap(sender: string, key: PoolKey, params: SwapParams): string {
    ...
    const functionSignature = "_beforeSwap(address,address,address,uint24,int24,address,bool,int256,uint160)";
    ...
    const encodedArgs = abiCoder.encode(
      ...
      [sender, key.currency0, key.currency1, key.fee, key.tickSpacing, key.hooks, params.zeroForOne, params.amountSpecified, params.sqrtPriceLimitX96]
    );
  ...
  encodeHookData(pm: PredicateMessage, msgSender: string, msgValue: BigNumber): string {
    ...
    const encoded = abiCoder.encode(
      ...
      [
        [pm.taskId, pm.expireByBlockNumber.toString(), pm.signerAddresses, pm.signatures],
        msgSender,
        msgValue.toString(),
      ]
    );
